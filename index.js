...
console.log(convertEvenToOdd([2, 2, 3, 4, 6, 5]));
// [1, 3, 3, 5, 5, 7, 7, 9, 9]
*/
...
const filterWords = function (arr) {
  const biggerWords = [];
  let index = 0;
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].length > 5) {
      biggerWords[index] = arr[i];
      index++;
    }
  }
  return biggerWords;
...
const capitalizeWords = function (arr) {
  const capitalized = [];
  for (let i = 0; i < arr.length; i++) {
    capitalized[i] = arr[i].toUpperCase();
  }
  return capitalized;
};
...
const podAndTeamAllocation = function (user) {
  const userDetails = { ...user };
  const min = 10000;
  const max = 999999;
  let randomNum = randomGenerator(min, max);
  while (idGenerated[randomGenerator] !== undefined) {
    randomNum = randomGenerator(min, max);
  }
  idGenerated[randomNum] = userDetails.name;
  userDetails.teamID = randomNum;
  return userDetails;
};
...
const checkForTeam = function (details) {
  const studentDetails = { ...details };
  const { team } = studentDetails;
  if (team === undefined) studentDetails.team = "A";
...
const {
  title,
  // authors: [personOne, personTwo]
  authors: [{ name: author1 }, { name: author2 = "Jagrut" }],
  publisher: { name: publisherName },
} = book;

// const { name: author1 } = personOne;
// const { name: author2 } = personTwo;
...
const sumOfAges = function (arr) {
  const personDetailsArr = [...arr];
  let sum = 0;
  for (const { age } of personDetailsArr) {
    sum += age;
  }
  return sum;
};
